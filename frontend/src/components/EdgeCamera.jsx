...
export default function EdgeCamera({ onAlert, onAlertTrigger }) {
  // State for UI Status Banner: 'loading' | 'monitoring' | 'alert' | 'error'
  const [appState, setAppState] = useState('loading');
  const [errorMessage, setErrorMessage] = useState('');
  const [personCount, setPersonCount] = useState(0);
  const [cooldownRemaining, setCooldownRemaining] = useState(0);

  // AI Confidence Threshold Setting (default 75% to eliminate false positives on clothes)
  const [confidenceThreshold, setConfidenceThreshold] = useState(0.75);

  // Refs for video, canvas overlay, and hidden snapshot canvas
  const videoRef = useRef(null);
  const visibleCanvasRef = useRef(null);
  const hiddenCanvasRef = useRef(null);

  // Refs for stateful lifecycle tracking across intervals
  ...
  // 1 & 2. Setup & AI Model Loading with High-Accuracy Model
  const initCameraAndAI = useCallback(async () => {
    try {
      ...
      // Load standard mobilenet_v2 for higher accuracy and fewer false positives
      const model = await cocoSsd.load({ base: 'mobilenet_v2' });
      modelRef.current = model;

      // Initialize webcam stream (640x480)
  ...
  // 3. Inference Loop (Runs every 1000ms with Smart Filtering)
  const startInferenceLoop = () => {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        // Step A: Filter by confidence threshold (e.g. >= 75%)
        const confidentPersons = rawPredictions.filter(
          p => p.class === 'person' && p.score >= currentThreshold
        );

        // Step B: Deduplicate overlapping/stacked ghost detections (IoU > 0.35)
        ...
        // 4. Draw Clean Cyan Bounding Boxes
        ...
        uniquePersons.forEach(prediction => {
          ...
          // Dark text inside cyan pill
          ctx.fillStyle = '#000000';
          ctx.fillText(label, x + 4, labelY + textHeight - 3);
        });

        // 5. Trigger Logic: Tailgating detected ONLY if 2+ unique real persons exist
        if (personsFound >= 2) {
  ...
  // 6, 7 & 8. Base64 Snapshot, API Dispatch & 4s Cooldown
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {/* 640x480 Video Viewport with Exact Overlay Canvas */}
        ...
        {/* Hidden Canvas for Base64 Snapshot Extraction */}
        <canvas
          ref={hiddenCanvasRef}
          width="640"
          height="480"
        ...
        {/* Controls Bar: Sensitivity Slider & Metadata */}
